...
app.post('/api/receberDadosLancamentosParcelaEstorno', async (req, res) => {
  ...
  try {
    ...
    // Verificando se os eventos são "lancamento_parcela_estorno_int" antes de inserir os dados
    if (dados.lancamentos.ITLA_CD_EVENTO !== 'lancamento_parcela_estorno_int') {
      throw new Error('O evento de lancamentos não é "lancamento_parcela_estorno_int"');
    }
    ...
    // Verificando se os eventos são "acordo_parcela_pagamento_int" antes de inserir os dados de 'lancamentosParcelas'
    if (Array.isArray(dados.lancamentos.lancamentosParcelas)) {
      if (!dados.lancamentos.lancamentosParcelas.every(parcela => parcela.ITLP_CD_EVENTO === 'lancamento_parcela_estorno_int')) {
        throw new Error('O evento de lancamentosParcelas não é "lancamento_parcela_estorno_int"');
      }
    }
    ...
    // Verificando se os eventos são "lancamento_parcela_estorno_int" antes de inserir os dados de 'pagamentos'
    if (Array.isArray(dados.lancamentos.pagamentos)) {
      if (!dados.lancamentos.pagamentos.every(pagamento => pagamento.ITPA_CD_EVENTO === 'lancamento_parcela_estorno_int')) {
        throw new Error('Um ou mais eventos de pagamentos não são "lancamento_parcela_estorno_int"');
      }
    }

    // Processando os dados de 'pagamentos'
    const pagamentos = dados.lancamentos.pagamentos;

    // Se 'pagamentos' for um array, iterar sobre ele
    if (Array.isArray(dados.lancamentos.pagamentos)) {
      await Promise.all(dados.lancamentos.pagamentos.map(async (pagamento, index) => {
...
app.listen(3015, () => console.log('Servidor rodando na porta 3015'));
