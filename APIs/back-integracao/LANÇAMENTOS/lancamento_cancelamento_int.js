...
app.post('/api/receberDadosLancamentoCancelamento', async (req, res) => {
  ...
  try {
    ...
    // Verificando se os eventos são "lancamento_cancelamento_int" antes de inserir os dados
    if (dados.lancamentos.ITLA_CD_EVENTO !== 'lancamento_cancelamento_int') {
      throw new Error('O evento de lancamentos não é "lancamento_cancelamento_int"');
    }
    ...
    // Verificando se os eventos são "lancamento_cancelamento_int" antes de inserir os dados de 'lancamentosParcelas'
    if (Array.isArray(dados.lancamentos.lancamentosParcelas)) {
      if (!dados.lancamentos.lancamentosParcelas.every(parcela => parcela.ITLP_CD_EVENTO === 'lancamento_cancelamento_int')) {
        throw new Error('O evento de lancamentosParcelas não é "lancamento_cancelamento_int"');
      }
    }
